...
from rl.encoding import action_mask, encode_aux, encode_obs
...
def make_policy_agent(
    ...
    def choose_action(state: GameState) -> int:
        board = torch.tensor(encode_obs(state), dtype=torch.float32, device=dev).unsqueeze(0)
        aux = torch.tensor(encode_aux(state), dtype=torch.float32, device=dev).unsqueeze(0)
        mask = torch.tensor(action_mask(state), dtype=torch.bool, device=dev).unsqueeze(0)
        logits, _ = policy(board, aux)
